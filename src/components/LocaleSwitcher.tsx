  ...
  return (
    <div className="relative inline-block text-left">
      <div ref={dropdownRef}>
        {currentFlag && (
          <div
            className="cursor-pointer text-white flex flex-row gap-2 items-center round"
            onClick={toggleMenu}
          >
            <img
              src={currentFlag.img}
              alt={currentFlag.id}
              className="w-[20px] h-[20px] object-center object-cover rounded-full"
            />
            ...
            {flags.map((item) => (
              ...
              >
                <img
                  src={item.img}
                  alt={item.id}
                  className="w-[20px] h-[20px] object-center object-cover"
                />
                <span>{item.text}</span>
              </div>
            ))}
